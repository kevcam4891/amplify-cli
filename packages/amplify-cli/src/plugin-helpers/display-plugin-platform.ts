...
export function displayScanInterval(context: Context, pluginPlatform: PluginPlatform){
    context.print.info(''); 
    displaySimpleString(context, 'Automatic plugin scan interval in seconds', 
        pluginPlatform.maxScanIntervalInSeconds.toString()); 
    context.print.info(''); 
}
...
export function displayPluginInfo(context: Context, pluginInfo: PluginInfo, group: string = ''){
    const title = `${pluginInfo.manifest.name}: ${pluginInfo.packageName}@${pluginInfo.packageVersion}`;
    context.print.info('');
    if(group.length > 0){
